...
class JBRefreshStateHeader: JBRefreshHeader {
    ...
    override var lastUpdatedTimeKey: String? {
        set(newValue) {
            ...
            if lastUpdatedTime != nil {
                ...
                // 3.显示日期
                lastUpdatedTimeLabel.text = String(format: "%@%@%@", Bundle.localizedString(JBRefreshHead.lastTimeText), isToday ? Bundle.localizedString(JBRefreshHead.dateTodayText) : "", time)
            } else {
                lastUpdatedTimeLabel.text = String(format: "%@%@", Bundle.localizedString(JBRefreshHead.lastTimeText), Bundle.localizedString(JBRefreshHead.noneLastDateText))
            }
...
extension JBRefreshStateHeader {
    override func prepare() {
        super.prepare()
        // 初始化间距
        labelLeftInset = JBRefreshConst.labelLeftInset
        // 初始化文字
        setTitle(Bundle.localizedString(JBRefreshHead.idleText), .Idle)
        setTitle(Bundle.localizedString(JBRefreshHead.pullingText), .Pulling)
        setTitle(Bundle.localizedString(JBRefreshHead.refreshingText), .Refreshing)
    }
