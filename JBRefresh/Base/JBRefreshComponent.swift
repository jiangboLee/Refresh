...
extension JBRefreshComponent {
    func addObservers() {
        ...
        scrollView?.addObserver(self, forKeyPath: JBRefreshKeyPath.contentOffset, options: options, context: nil)
        scrollView?.addObserver(self, forKeyPath: JBRefreshKeyPath.contentSize, options: options, context: nil)
        pan = scrollView?.panGestureRecognizer
        pan?.addObserver(self, forKeyPath: JBRefreshKeyPath.panState, options: options, context: nil)
    }
    func removeObservers() {
        superview?.removeObserver(self, forKeyPath: JBRefreshKeyPath.contentOffset)
        superview?.removeObserver(self, forKeyPath: JBRefreshKeyPath.contentSize)
        pan?.removeObserver(self, forKeyPath: JBRefreshKeyPath.panState)
        pan = nil
    }
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        // 这个就算看不见也需要处理
        if keyPath == JBRefreshKeyPath.contentSize {
        ...
        if keyPath == JBRefreshKeyPath.contentOffset {
            scrollViewContentOffsetDidChange(change)
        } else if keyPath == JBRefreshKeyPath.panState {
